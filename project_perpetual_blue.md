...
- [ ] **Create Sean & Elise logins** — add Supabase Auth users (**`captainsean@perpetualbluebvi.com`** = captain, `elise@perpetualbluebvi.com` = crew), then link each with `UPDATE crm_users SET supabase_uid = '<uuid>' WHERE email = '...'`
  - ⚠️ **Corrected 2026-08-29:** `migration_010_roles.sql` seeded Sean as `sean@` but his real Zoho mailbox is **`captainsean@`** (matches `migration_004_crew.sql`). Migration file fixed; **if migration_010 already ran, the live `crm_users` row still says `sean@`** — run `UPDATE crm_users SET email='captainsean@perpetualbluebvi.com' WHERE email='sean@perpetualbluebvi.com';`
  - ⚠️ **`elise@perpetualbluebvi.com` does not exist as a mailbox** — Zoho only has `charters@` and `captainsean@`. Create it or change her row before building her login.
