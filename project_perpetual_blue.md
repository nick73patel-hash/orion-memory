...
- **Crew:** Captain **Sean Powell** (South African) — mobile **+1 954 807 1333** (US/Fort Lauderdale number), `captainsean@perpetualbluebvi.com`. **Internal labour rate $30/hr (set 2026-08-30) — ⚠️ NOTIONAL ONLY: never booked, never charged, never exported to Digits.** He's salaried, so booking it would double-count real money. It exists purely to answer "how long did that job take and what was it worth" for estimating the next one. **Real project cost = materials + subcontractors only.**
- ✅ **Chef: HENRY KLOPPER (he/him), South African** — confirmed 2026-08-30, replacing Elise McNabb who departed 2026-08-29. Email **`chefhenry@perpetualbluebvi.com`** (owner's choice, matches the `captainsean@` pattern). ⚠️ **The mailbox does NOT exist yet** — Zoho Mail Free covers 5 users and currently holds 2 (`charters@`, `captainsean@`), so a third costs **$0**. Create it at **`mailadmin.zoho.com`** — NOT `admin.zoho.com`, which redirect-loops on a Mail Free account. **Start date still to be captured**; being entered in the CRM Wed 2026-09-02 (`migration_027`).
  - ⚠️ **He is South African, same as Sean — so the same BVI work-permit path.** Sean's has been *in process since ~July and still isn't issued*: that's the realistic timeline signal for Henry. Permits are **person-specific**, so Elise's cannot be transferred — withdraw hers and file Henry's fresh, as early as possible. (Her past records stay: the $108.97 SJU→EIS flight on 2026-07-28 is a real historical expense.)
- **Current ownership:** Florida LLC — **SVFP57 LLC**
- **Owners (3, confirmed 2026-08-28):** **Nick Patel**, **David Brandt**, **Derek Yurosek**.
- ⭐ **CAPITAL NOW LIVES IN THE CRM (2026-08-30).** Migration 028 applied; opening balances loaded and **tie-out verified by Nick**. 🚫 **`Projects\perpetual-blue\partner-capital-tracker.md` IS RETIRED — do not update it.** Two live sources of truth is exactly how the MHG $10,782 double-count happened. Keep it as a historical artefact only. QuickBooks accts 340/350 are history too; **Digits is the GL from 2026-08-30**.
- ⭐ **OWNERSHIP IS FIXED AT ONE THIRD EACH, REGARDLESS OF CAPITAL CONTRIBUTED (confirmed 2026-08-30).** This is the decision that defines what the capital module is *for*.
  - **Contributing more never buys more of the boat.** So an imbalance is an **obligation, not equity** — the ~**$28,217.62** gap is either **trued up in cash** or recorded as a **member loan** to be repaid. There is no third option where over-contribution quietly becomes more ownership, because ownership cannot move.
  - ✅ **DECIDED 2026-08-30: book everything as CAPITAL CONTRIBUTION, not a member loan.** Nick's reasoning: the short owners will always catch up, so the imbalance is temporary and self-correcting. This also avoids member-loan formalities (repayment terms, interest, imputed-interest rules). The `instrument` field still exists and stays `NOT NULL`, but the answer is `equity` unless something changes.
  - 📌 **So the capital module's PRIMARY JOB is making the gap visible** so it actually does get closed. The headline number is each owner's shortfall against the baseline, not a ledger dump.
  - ⚠️ **Revisit if the assumption breaks** — if an owner stops catching up, or if there's an exit or sale before the gap closes, capital accounts under a fixed-thirds split won't by themselves record who is owed what. Not a problem today; worth a look at settlement time.
  - **Nick's reporting framing stands and is the right one for fixed thirds:** highest contributor = baseline, others shown as "behind." (Equal-share view stays computable but is secondary.)
...
- **Cards: Mercury IO** — ⚠️ **corrected 2026-08-29: IO is a *balance-secured charge card*, NOT a true revolving credit card.** Mercury markets it as a "business credit card," but the limit is tied to your Mercury balance — it extends no actual credit. $0 annual fee, 1.5% cash back, unlimited free employee cards (owner, Captain Sean, Chef Elise) with per-card limits, native QuickBooks sync. Good as the operating/spend card, not as a credit line.
- **For an actual revolving credit card: Chase Ink Business Cash** ($0 fee, 5% phone/internet/office supply, 2% gas/dining, big welcome bonus, Visa = better island acceptance). **Applied for ~Aug 2026 — confirmation email received, still pending as of Aug 29.** Ink applications routinely sit 7–10 business days; a young multi-member LLC operating in the BVI is a manual-review profile. Chase has a **business reconsideration line** if it stalls.

> 🚨 **THE 3% FOREIGN TRANSACTION TRAP (learned 2026-08-29).** **Both Mercury IO and Chase Ink Business *Cash* charge 3% FTF on non-USD purchases.** This is invisible in the BVI — **BVI runs on US dollars** — but it hits hard in the **Grenada/SVG offseason**, where Grenada, Carriacou, Union Island and the Tobago Cays all use **XCD**. Every provisioning run, fuel stop and customs fee in Aug 2026 took a 3% haircut.
> - **No-FTF options:** **Capital One Spark Cash Plus** (2% flat, no FTF, Visa — best fit for island spend) · **Chase Ink Business *Preferred*** (no FTF, but $95/yr). Note Ink *Cash* ≠ Ink *Preferred* on this.
> - **Plan chosen:** keep Ink Cash for domestic spend (5% on Starlink/phone/internet is worth it), add a no-FTF Visa for Sean's non-USD spend. Don't hand Ink Cash to crew for XCD purchases.
> - **Tell Sean:** at any card terminal offering to charge in **USD instead of local currency, always decline** — that's dynamic currency conversion, typically 3–7% worse than the Visa rate. Pay in XCD.
> - **Revolut/Wise are NOT needed for this.** They solve non-USD *wires* (Mercury charges 1% FX; USD wires are free). Card FTF is solved by a no-FTF card, not a new bank.
- 📌 **NEXT SESSION (planned 2026-08-31): set up Digits.** Sign-up, connect the Mercury account, and establish the real chart of accounts **inside Digits** — not in the CRM. **Decision 2026-08-30: the CRM will NOT hold a chart of accounts.** Digits owns the ledger; the CRM feeds it operational data (`expenses` already uses free-text categories) and maps categories → Digits accounts at the export boundary. A mirrored local chart would just be a second thing to maintain and drift.
- **Accounting: Digits** — AI-powered ledger. ✅ **CONFIRMED 2026-08-30: Digits is the general ledger going forward, not QuickBooks.** Build all export mapping against Digits. Receipts uploaded via CRM → categorized → pushed to Digits.
  - **Digits Connect API verified 2026-08-30:** real, launched Oct 2025, self-serve, OAuth 2.0, `POST /v1/source/transactions`, **idempotent on `externalId`** (which is what makes a safe export possible). **No confirmed bulk-import path** for the historical ~$831K of capital — that migrates some other way.
  - ⚠️ **The CRM's seeded chart of accounts is ANOTHER VESSEL'S REAL BOOKS** — "Shore Financial Checking/Escrow", "Partner Capital — Alexander", "Partner Capital — ITR", "Due to Nex Gen Yachting", "Term Loan from Shore Financial", plus fake owner "Nigel & Brenda Alexander" and cards "Frontier Card"/"United Visa" (all in `migration_002_seed.sql`). Not placeholders — a real third party's data. **Set to `is_active = false` 2026-08-30.** Note `transactions.account_code` is plain `text` with **no FK to `accounts`**, so bad codes never error.
  - **QuickBooks accts 340/350 are now history only** — they're how the existing ~$831K was booked and how the partner reconciliation was done. Don't build new mapping against them.
...
> - **The broken policy:** `CREATE POLICY "Admins can read all rows" ON crm_users ... USING (EXISTS (SELECT 1 FROM crm_users ...))` — a policy *on* `crm_users` that queries `crm_users`. Postgres raises **`42P17: infinite recursion detected`** on any authenticated read.
> - **How it presents — nothing says "error".** `getCurrentCrmUser()` reads `crm_users` via the *user-session* client, so it returns null. `app/(crm)/layout.tsx` then falls back to `role = crmUser?.role ?? 'owner_readonly'`, and `owner_readonly.canEdit = false`. **Every signed-in user is silently demoted to read-only:** pages render and data loads (those reads use the service-role client, which bypasses RLS), but edit controls vanish and every write route 403s. It reads as "saves don't work," never as an auth error.
> - **Fix:** `supabase/migration_022_fix_crm_users_rls.sql` — policies built on `is_admin()` (already `SECURITY DEFINER`, so it bypasses RLS and cannot recurse), `SET search_path = public` pinned on both helper functions, plus an email-join `UPDATE` that links every `crm_users` row to its `auth.users` row without hand-copying UUIDs. **Ran successfully 2026-08-29 — saving restored.**
> - **`migration_010`'s seed is now `ON CONFLICT (email) DO NOTHING`.** As written it aborted on `UNIQUE(email)`, and in a single-transaction SQL editor that rolls back the *entire file* — policies included.
> - ⚠️ **THE RULE: any hot-fix applied straight to the Supabase DB must be written back into a migration file the same day**, or the next person to run the migration silently regresses it. This has now cost two debugging sessions for one bug.
...
- [ ] **⭐ CAPTAIN PERMISSION POLICY — decided 2026-08-30, build before Sean gets a login.**
  - **Sean SEES EVERYTHING.** Nick explicitly overrode Orion's suggestion to hide other crew's payroll and owner capital — he wants full visibility. So `captain` gets **all modules** plus `seePayroll`, `seeAnalytics`, `seeReconciliation` = true.
  - **Principle for editing: "Sean edits what he does; Nick edits what the business is."**
  - **CAN EDIT:** maintenance jobs / history / schedule · captain's log · his time entries · expenses (create + edit) · fuel · inventory + bar inventory · itineraries + stops · guest preferences · monthly tasks · ports · galley/recipes
  - **VIEW ONLY:** charters (bookings are a commercial act) · brokers + commissions · rate cards / pricing · crew records · payroll · documents (insurance, registration) · reconciliation · analytics · vessel settings · users/roles · owner capital
  - **✅ NO DELETES AT ALL** — the one Orion suggestion Nick adopted. Sean may edit and complete records but never delete any. Deletion is irreversible and this system has no audit trail; a mis-entered fuel log can be corrected, it never needs removing.
  - ⚠️ **The blocker: the granular flags already exist but are NOT enforced.** `captain` already has `editMaintenance`/`editCaptainsLog`/`editTasks`/`editInventory` in `lib/roles.ts`, but `requireEditPermission()` only checks the blanket `canEdit: true` — so today a captain could edit anything via the API. The flags are decorative. **The work is the enforcement layer**, plus a `canDelete` flag.
  - Related: the auth pass (`9135c6d`) added `requireAuth()` / `requirePermission(flag)` / `authErrorResponse()` to `lib/roles-server.ts` and mapped which endpoints need role-scoping — build on that.
- [ ] **⭐ CHEF / `crew` PERMISSION POLICY — decided 2026-08-30. Needed by Wed 2026-09-02, chef gets entered then.** (Current config is contradictory: `canEdit: false` but `editInventory: true`.)
  - **SCOPED, not full visibility** — Nick's explicit choice, deliberately different from Sean. The chef's job doesn't require broker commissions or owner capital, and that access is awkward to withdraw later.
  - **CAN EDIT:** galley / recipes / menus / provisioning · galley + bar inventory · guest preference notes (allergies, dietary detail she gathers) · **create maintenance jobs only** (she finds the broken oven — create, but not edit or complete)
  - **VIEW ONLY:** charters + calendar (who's aboard, when) · guests + preferences (**allergies are safety-critical**) · itineraries + ports (provisioning depends on location) · maintenance (status of what she reported)
  - **CANNOT SEE:** expenses · rates · broker commissions · payroll · owner capital · analytics · documents · reconciliation · crew records
  - **✅ NO DELETES** — same rule as the captain.
- [ ] **Create Sean's CRM login** — add a Supabase Auth user for **`captainsean@perpetualbluebvi.com`** (captain), then link with `UPDATE crm_users SET supabase_uid = '<uuid>' WHERE email = 'captainsean@perpetualbluebvi.com';` **Do the permission policy above FIRST.**
  - ⚠️ **Corrected 2026-08-29:** `migration_010_roles.sql` seeded Sean as `sean@` but his real Zoho mailbox is **`captainsean@`** (matches `migration_004_crew.sql`). Migration file fixed; the live `crm_users` row was **updated and confirmed** 2026-08-29.
  - ⚠️ **Elise's login is CANCELLED — she departed 2026-08-29.** `elise@perpetualbluebvi.com` was never created as a mailbox, so nothing to delete there. **Her `crm_users` row still exists and should be deactivated** (don't hard-delete — it's referenced by historical records). Rebuild the crew login for the next chef when hired.
...
>
> Still open on the mail side: **display name + signature on both mailboxes** (in progress 2026-08-29), mobile apps, and the DMARC bump to `p=quarantine`. **2FA — deliberately back-burnered by Nick 2026-08-29**, don't keep raising it. **`elise@` is moot** — never created, and she departed 2026-08-29.
>
> ⚠️ **Unverified: what display name `charters@` sends as.** Orion cannot read the mailbox. The account holder is recorded as "Nikunj" — if that's what's in Zoho, brokers and guests see *Nikunj* on the From line rather than anything branded. **Check at `mailadmin.zoho.com` and decide deliberately** (personal name vs. "Perpetual Blue Charters").
